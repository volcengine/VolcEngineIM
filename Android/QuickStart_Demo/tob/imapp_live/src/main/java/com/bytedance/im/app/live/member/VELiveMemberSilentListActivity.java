...
public class VELiveMemberSilentListActivity extends Activity {
    ...
    public static void start(Activity activity, long conversationShortId) {
        Intent intent = new Intent(activity, VELiveMemberSilentListActivity.class);
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        ((TextView) findViewById(R.id.tv_title)).setText("禁言名单");
        findViewById(R.id.back).setOnClickListener(v -> finish());
        RecyclerView recyclerView = findViewById(R.id.user_list);
        recyclerView.setLayoutManager(new LinearLayoutManager(this));
        recyclerView.setItemAnimator(null);
        conversationShortId = getIntent().getLongExtra(CONVERSATION_SHORT_ID, 0L);
        adapter = new VELiveMemberListAdapter(this, memberWrapper -> showOperation(memberWrapper));
        recyclerView.setAdapter(adapter);
        findViewById(R.id.tv_more).setVisibility(View.VISIBLE);
        findViewById(R.id.tv_more).setOnClickListener((view) -> VEEditCommonActivity.startForResult(this, "添加禁言黑名单", "",19, REQUEST_EDIT_UID));
    ...
    private void showOperation(VELiveMemberWrapper memberWrapper) {
        ...
        dialogInfo.add(new android.util.Pair("移出成员", (VELiveGroupDialogUtils.BottomInputDialogListener) (v, text) -> BIMClient.getInstance().getService(BIMLiveExpandService.class).removeLiveGroupMemberSilentList(conversationShortId, Collections.singletonList(memberWrapper.getMember().getUserID()), new BIMSimpleCallback() {
            public void onSuccess() {
                Toast.makeText(VELiveMemberSilentListActivity.this, "移出禁言黑名单成功" + BIMUINameUtils.getShowNameInGroup(memberWrapper.getMember(),memberWrapper.getFullInfo()), Toast.LENGTH_SHORT).show();
                adapter.remove(memberWrapper.getMember().getUserID());
            }

            public void onFailed(BIMErrorCode code) {
                Toast.makeText(VELiveMemberSilentListActivity.this, "移出禁言黑名单失败" + BIMUINameUtils.getShowNameInGroup(memberWrapper.getMember(),memberWrapper.getFullInfo()), Toast.LENGTH_SHORT).show();
            }

        })));
        dialogInfo.add(new android.util.Pair("取消", (VELiveGroupDialogUtils.BottomInputDialogListener) (v, text) -> Toast.makeText(VELiveMemberSilentListActivity.this, "取消", Toast.LENGTH_SHORT).show()));
        VELiveGroupDialogUtils.showBottomMultiItemDialog(this, dialogInfo);
    }

    protected void loadData() {
        BIMClient.getInstance().getService(BIMLiveExpandService.class).getLiveGroupMemberSilentList(conversationShortId, cursor, 20, new BIMResultCallback<BIMLiveMemberListResult>() {
            @Override
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == RESULT_OK && requestCode == REQUEST_EDIT_UID) {
            ...
            BIMClient.getInstance().getService(BIMLiveExpandService.class).addLiveGroupMemberSilentList(conversationShortId, uidList, blocktime, new BIMSimpleCallback() {
                @Override
                public void onSuccess() {
