//
//  VEIMDemoSilentListViewController.m
//  ByteBusiness
//
...
@interface VEIMDemoSilentListViewController ()<VEIMDemoUserSelectionControllerDelegate>
@property (nonatomic, assign) long long nextCursor;

@end

@implementation VEIMDemoSilentListViewController
...
- (void)viewDidLoad {
    ...
    self.isShowSilentMark = YES;
    self.delegate = self;
}

- (void)rightClicked:(id)sender{
    kWeakSelf(self);
    VEIMDemoInputController *vc = [[VEIMDemoInputController alloc] initWithTitle:@"禁言名单" text:nil maxWordCount:100 editable:YES handler:^(NSString * _Nonnull text) {
        long long userID = text.longLongValue;
        [[BIMClient sharedInstance] addLiveGroupMemberSilentList:weakself.conversation.conversationID uidList:[NSSet setWithObject:@(userID)] blockTime:0 completion:^(BIMError * _Nullable error) {
            if (error) {
                [BIMToastView toast:[NSString stringWithFormat:@"禁言失败: %@",error.localizedDescription]];
            } else {
...
{
    kWeakSelf(self)
    [[BIMClient sharedInstance] getLiveGroupMemberSilentList:self.conversation.conversationID cursor:cursor count:100 completion:^(NSArray<id<BIMMember>> * _Nullable members, BOOL hasMore, long long nextCursor, BIMError * _Nullable error) {
...
- (void)userSelectVC:(VEIMDemoUserSelectionController *)vc didLongPressUser:(nonnull VEIMDemoUser *)user indexPath:(nonnull NSIndexPath *)indexPath
{
    ...
    UIAlertAction *remove = [UIAlertAction actionWithTitle:@"移除成员" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        [[BIMClient sharedInstance] removeLiveGroupMemberSilentList:self.conversation.conversationID uidList:[NSSet setWithObject:@(user.userID)] completion:^(BIMError * _Nullable error) {
            if (error) {
